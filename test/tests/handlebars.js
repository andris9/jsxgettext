...
exports['test handlebars'] = function (assert, cb) {
  ...
  fs.readFile(inputFilename, "utf8", function (err, source) {
    ...
    assert.equal(result.split(/msgid ".+"/).length, 7, 'exactly six strings are found');
    ...
    assert.notEqual(result.indexOf('msgid "a context argument"'), -1, 'result contains the sixth string');
    cb();
  });
};
